...
const Hero = () => {
  ...
  useEffect(() => {
    if (sectionRef.current) {
      gsap.fromTo(sectionRef.current, { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: 1, ease: "power3.out" });
    }
    if (headlineRef.current) {
      gsap.fromTo(headlineRef.current, { opacity: 0, y: 60, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: 1.1, delay: 0.2, ease: "power4.out" });
    }
    if (subRef.current) {
      gsap.fromTo(subRef.current, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1, delay: 0.5, ease: "power2.out" });
    }
    if (btnsRef.current) {
      gsap.fromTo(btnsRef.current.children, { opacity: 0, y: 30, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: 0.8, stagger: 0.15, delay: 0.8, ease: "back.out(1.7)" });
    }
    if (imgRef.current) {
      gsap.fromTo(imgRef.current, { opacity: 0, x: 80, scale: 0.92, rotate: 8 }, { opacity: 1, x: 0, scale: 1, rotate: 0, duration: 1.2, delay: 0.7, ease: "expo.out" });
    }
  }, []);
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-x-6 gap-y-10 items-center">
          <div className='mt-14 lg:mt-0'>
            ...
            >
              ...
              <button className="py-2 px-6 text-[#014bac] dark:text-white rounded-md border border-[#014bac] dark:border-gray-200 text-base hover:border-[#014bac] hover:bg-[#014bac] hover:text-white transition-all duration-500 font-medium">
                Partner with Us
              </button>
            </div>
          </div>
